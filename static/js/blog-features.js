...
(function () {
  ...
  function initShareButtons() {
    ...
    container.innerHTML =
      ...
      '<button class="share-btn share-btn--twitter" aria-label="Share on X / Twitter">' +
      ...
      '<button class="share-btn share-btn--linkedin" aria-label="Share on LinkedIn">' +
        ...
        '<span>share</span>' +
      '</button>' +
      '<button class="share-btn share-btn--copy" aria-label="Copy link to clipboard">' +
    ...
    // Check if device supports native share (better for mobile apps)
    var isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);

    // Twitter / X — structured tweet with unicode
    ...
    // LinkedIn — use native share on mobile (opens LinkedIn app), fallback to web
    container.querySelector('.share-btn--linkedin').addEventListener('click', function () {
      if (isMobile && navigator.share) {
        navigator.share({
          title: cleanTitle,
          text: cleanTitle + ' — ' + author,
          url: pageUrl
        }).catch(function () {});
      } else {
