...
The first goal is to integrate PIC with existing OAuth infrastructure: exchange an OAuth token, evaluate the authority it represents, issue the initial PIC PCA COSE and PIC Continuity COSE, and return the PIC Token JWT required to begin a PIC execution.

```text
OAuth token  →  PIC-X Trust Plane  →  PIC Token JWT
```
